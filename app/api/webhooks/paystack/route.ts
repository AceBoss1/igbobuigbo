...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await notifySuperadmins(
      ...
      `Paystack confirms ${reference} (₦${(amountKobo / 100).toLocaleString()}) succeeded, but processing threw an error. Needs manual review.`,
    ).catch(() => {});
  }
...
async function flagStuckRegistration(reference: string, amountKobo: number, email?: string) {
  ...
  await notifySuperadmins(
    '⚠️ Paid registration may be stuck',
    `₦${(amountKobo / 100).toLocaleString()} charged for ${reference}${email ? ` (${email})` : ''}, but no member record exists yet. Likely an interrupted signup — needs manual follow-up.`,
  ).catch(() => {});
}
